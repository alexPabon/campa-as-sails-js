...
without necessarily having a billing card.`
  ...
  customCreate: async function(userData) {
  ...
  /**
   * Change the language in the 'user.lang' table
   * @param lang
   * @param userId
   * @returns {Promise<*>}
   */
  changeLanguageUser: async function(lang, userId){
    const languages = sails.config.i18n.locales;

    if(!languages.includes(lang)){
      throw new Error(sails.__('language not registered'));
    }

    let updateUserLang = await User.updateOne({id:userId}).set({lang:lang});
